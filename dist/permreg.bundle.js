...
`;var J=null;function lt(r){J=r}var w=r=>String(r??"").replace(/&/g,"&amp;").replace(/</g,"&lt;").replace(/>/g,"&gt;").replace(/"/g,"&quot;");function et(r){let e=document.createElement("template");return e.innerHTML=r.trim(),e.content.firstElementChild}function I(r,e){let n=J.querySelector(".pr-toasts");n||(n=et('<div class="pr-toasts" role="status" aria-live="polite"></div>'),J.appendChild(n));let l=et(`
    <div class="pr-toast pr-toast--${r}">
    ...
    </div>`);l.querySelector(".pr-msg").textContent=e,l.addEventListener("click",u=>{let m=u.target.closest("[data-tact]");m&&(m.dataset.tact==="copy"?navigator.clipboard.writeText(e).catch(()=>{}):l.remove())}),n.appendChild(l),r==="ok"&&setTimeout(()=>l.remove(),2e3),r==="warn"&&setTimeout(()=>l.remove(),3e3)}function M({title:r,message:e,inputValue:n,okLabel:l,danger:u}){return new Promise(m=>{let _=n!==void 0,d=et(`
      ...
      </div>`);d.querySelector("h4").textContent=r,e!=null&&(d.querySelector(".pr-modal-msg").textContent=e);let k=d.querySelector("input");k&&(k.value=n),d.querySelector('[data-mact="ok"]').textContent=l||"OK";let S=p=>{document.removeEventListener("keydown",O,!0),d.remove(),m(p)},b=()=>S(_?null:!1),j=()=>S(_?k.value.trim():!0),v=!1;d.addEventListener("mousedown",p=>{v=p.target===d}),d.addEventListener("click",p=>{if(p.target===d){v&&b();return}let q=p.target.closest("[data-mact]");q&&(q.dataset.mact==="ok"?j():b())});let O=p=>{p.isComposing||p.keyCode===229||(p.key==="Escape"?(p.stopPropagation(),b()):p.key==="Enter"&&(p.metaKey||p.ctrlKey||_&&p.target===k)&&j())};document.addEventListener("keydown",O,!0),J.appendChild(d),(k||d.querySelector('[data-mact="ok"]')).focus(),k&&k.select()})}var ft=3e4;function gt(r,e){if(window.__permregSource=Object.assign({},r,{ver:e}),r.dev)fetch(r.base+"/permreg.bundle.js?v="+encodeURIComponent(e)).then(n=>{if(!n.ok)throw new Error("HTTP "+n.status);return n.text()}).then(n=>{(0,eval)(n)}).catch(n=>I("err","\u81EA\u52D5\u66F4\u65B0\u306B\u5931\u6557\u3057\u307E\u3057\u305F \u2014 "+(n&&n.message||n)));else{let n=document.getElementById("permreg-script");n&&n.remove();let l=document.createElement("script");l.id="permreg-script",l.src=r.base+"/permreg.bundle.js?v="+encodeURIComponent(e),l.onerror=()=>I("err","\u81EA\u52D5\u66F4\u65B0\u306B\u5931\u6557\u3057\u307E\u3057\u305F (script load error)"),document.body.appendChild(l)}}function pt(r){window.__permregWatcher&&clearInterval(window.__permregWatcher),window.__permregOnVisible&&document.removeEventListener("visibilitychange",window.__permregOnVisible);let e=window.__permregSource;if(!e||!e.base)return;let n=!1,l="";async function u(){if(n)return;let m="";try{let d=await fetch(e.base+"/version.txt?t="+Date.now(),{credentials:"same-origin"});if(!d.ok)return;m=(await d.text()).trim()}catch{return}if(!m||m===r||m===l)return;if(n=!0,await M({title:"\u66F4\u65B0\u304C\u3042\u308A\u307E\u3059",message:"\u65B0\u3057\u3044\u30D0\u30FC\u30B8\u30E7\u30F3 "+m+" \u304C\u914D\u4FE1\u3055\u308C\u3066\u3044\u307E\u3059(\u5B9F\u884C\u4E2D: "+r+")\u3002\u4ECA\u3059\u3050\u66F4\u65B0\u3057\u307E\u3059\u304B? \u7DE8\u96C6\u9014\u4E2D\u306E\u5165\u529B\u306F\u5931\u308F\u308C\u307E\u3059\u3002",okLabel:"\u4ECA\u3059\u3050\u66F4\u65B0"})){gt(e,m);return}l=m,n=!1}window.__permregWatcher=setInterval(()=>{document.hidden||u()},ft),window.__permregOnVisible=()=>{document.hidden||u()},document.addEventListener("visibilitychange",window.__permregOnVisible),window.__permregCheckNow=u}(()=>{"use strict";let r=document.getElementById(t);r&&r.remove();let e={view:"master",l1:[],l2:[],selectedL1:null,ready:!1,busy:!1};function n(){try{if(window._spPageContextInfo&&window._spPageContextInfo.webAbsoluteUrl)return window._spPageContextInfo.webAbsoluteUrl}catch{}let o=location.href.match(/^(https:\/\/[^/]+(?:\/(?:sites|teams)\/[^/]+)?)/);return o?o[1]:location.origin}Z(localStorage.getItem(Y)||n());async function l(){e.ready=!!await X(y)&&!!await X(h)}async function u(){let[o,a]=await Promise.all([P(x(y)+"/items?$select=Id,Title,SortOrder,Active&$orderby=SortOrder,Id&$top=4999"),P(x(h)+"/items?$select=Id,Title,SortOrder,Active,Level1/Id&$expand=Level1&$orderby=SortOrder,Id&$top=4999")]);e.l1=o.value||[],e.l2=a.value||[],e.selectedL1&&!e.l1.some(i=>i.Id===e.selectedL1)&&(e.selectedL1=null),!e.selectedL1&&e.l1.length&&(e.selectedL1=e.l1[0].Id)}let m=o=>o.reduce((a,i)=>Math.max(a,i.SortOrder||0),0)+10,_=(o,a)=>D(x(o)+"/items",a),d=(o,a,i)=>V(x(o)+"/items("+a+")",i),k=(o,a)=>rt(x(o)+"/items("+a+")");async function S(o,a,i,s){let f=a.indexOf(i),L=f+s;if(L<0||L>=a.length)return;let z=a.map(c=>c.SortOrder);if(z.some((c,C)=>c==null||z.indexOf(c)!==C))for(let c=0;c<a.length;c++)a[c].SortOrder=(c+1)*10,await d(o,a[c].Id,{SortOrder:a[c].SortOrder});let T=a[f],g=a[L];await d(o,T.Id,{SortOrder:g.SortOrder}),await d(o,g.Id,{SortOrder:T.SortOrder})}let b=document.createElement("div");b.id=t;let j=document.createElement("style");j.textContent=st,b.appendChild(j);let v=document.createElement("div");v.className="pr-app",b.appendChild(v),document.body.appendChild(b),lt(b);function O(o){let a=b.querySelector(".pr-status");a&&(a.textContent=o)}async function p(o,a){if(!e.busy){e.busy=!0,v.style.opacity="0.55",v.style.pointerEvents="none",O(o+"\u2026");try{await a(),O(o+" \u5B8C\u4E86")}catch(i){O("\u30A8\u30E9\u30FC: "+i.message),I("err",o+"\u306B\u5931\u6557\u3057\u307E\u3057\u305F \u2014 "+i.message)}finally{e.busy=!1,v.style.opacity="",v.style.pointerEvents=""}}}function q(){let o=s=>e.l2.filter(f=>f.Level1&&f.Level1.Id===s),a=e.l1.find(s=>s.Id===e.selectedL1),i=(s,f,L)=>`
      <div class="pr-row${s.Active===!1?" off":""}${L||""}" data-kind="${f}" data-id="${s.Id}">
        ...
        <span class="pr-name" ${f==="l1"?'data-act="select"':""} title="${w(s.Title)}">${w(s.Title)}${f==="l1"?`<span class="pr-childcount">${o(s.Id).length}</span>`:""}</span>
      ...
      <div class="pr-cols">
        <div class="pr-col">
          ...
          <div class="pr-rows">${e.l1.map(s=>i(s,"l1",s.Id===e.selectedL1?" sel":"")).join("")||'<div class="pr-empty">\u672A\u767B\u9332</div>'}</div>
        </div>
        <div class="pr-col">
          ...
          <div class="pr-rows">${o(a.Id).map(s=>i(s,"l2")).join("")||'<div class="pr-empty">\u672A\u767B\u9332</div>'}</div>`:'<div class="pr-empty">\u5DE6\u3067\u7B2C1\u968E\u5C64\u3092\u9078\u629E\u3057\u3066\u304F\u3060\u3055\u3044</div>'}
        </div>
      </div>`:`
        <div class="pr-hero">
      ...
      </div>`}function ct(){let o=window.__permregSource&&window.__permregSource.base||"\u76F4\u63A5\u5B9F\u884C(\u57CB\u3081\u8FBC\u307F/\u958B\u767A\u30B3\u30F3\u30BD\u30FC\u30EB)",a=localStorage.getItem(H)==="local",i=localStorage.getItem(Q)||W;return`
      <div class="pr-settings">
        <h4>\u8A2D\u5B9A</h4>
        <div class="pr-kv">\u30D0\u30FC\u30B8\u30E7\u30F3: <code>${w(R)}</code> / \u4ECA\u56DE\u306E\u8AAD\u8FBC\u5143: <code>${w(o)}</code></div>
        <div class="pr-field">
        ...
        <div class="pr-field">
          ...
          <input type="text" class="pr-input" id="pr-dev-base" value="${w(i)}" placeholder="${w(W)}">
      ...
      </div>`}function B(){let o={users:dt,master:q,settings:ct},a=(i,s,f)=>`
      <button class="pr-nav-item${e.view===i?" active":""}" data-act="nav" data-view="${i}">
      ...
      <div class="pr-status">${e.ready?"\u6E96\u5099OK":"\u30DE\u30B9\u30BF\u30EA\u30B9\u30C8\u672A\u4F5C\u6210"} / ${w(R)}</div>`}async function E(){await l(),e.ready&&await u(),B()}v.addEventListener("change",o=>{let a=o.target;if(a.id==="pr-weburl"){Z(a.value.trim()),localStorage.setItem(Y,tt());return}if(a.dataset.act==="active"){let i=a.closest(".pr-row"),s=i.dataset.kind==="l1"?y:h;p("\u66F4\u65B0",async()=>{await d(s,+i.dataset.id,{Active:a.checked}),await E()})}}),v.addEventListener("keydown",o=>{o.key==="Enter"&&(o.isComposing||o.keyCode===229||(o.target.id==="pr-add-l1"&&v.querySelector('[data-act="add-l1"]').click(),o.target.id==="pr-add-l2"&&v.querySelector('[data-act="add-l2"]').click()))}),v.addEventListener("click",async o=>{let a=o.target.closest("[data-act]");if(!a)return;let i=a.dataset.act,s=a.closest(".pr-row"),f=s&&s.dataset.kind,L=s&&+s.dataset.id,z=f==="l1"?y:h,F=f==="l1"?e.l1:e.l2.filter(g=>g.Level1&&g.Level1.Id===e.selectedL1),T=F&&F.find(g=>g.Id===L);if(i==="close"){b.remove();return}if(i==="nav"){e.view=a.dataset.view,B();return}if(i==="reload"){p("\u518D\u8AAD\u8FBC",E);return}if(i==="setup"){p("\u30BB\u30C3\u30C8\u30A2\u30C3\u30D7",async()=>{await it(O),await E(),I("ok","\u30DE\u30B9\u30BF\u30EA\u30B9\u30C8\u3092\u4F5C\u6210\u3057\u307E\u3057\u305F")});return}if(i==="select"){e.selectedL1=L,B();return}if(i==="save-settings"){let g=v.querySelector('input[name="pr-src"][value="local"]').checked,c=v.querySelector("#pr-dev-base").value.trim().replace(/\/+$/,"");g?(localStorage.setItem(H,"local"),localStorage.setItem(Q,c||W)):localStorage.removeItem(H),I("ok","\u4FDD\u5B58\u3057\u307E\u3057\u305F\u3002\u6B21\u56DE\u306E\u30D6\u30C3\u30AF\u30DE\u30FC\u30AF\u30EC\u30C3\u30C8\u8D77\u52D5\u304B\u3089\u53CD\u6620\u3055\u308C\u307E\u3059");return}if(i==="add-l1"||i==="add-l2"){let c=v.querySelector(i==="add-l1"?"#pr-add-l1":"#pr-add-l2").value.trim();if(!c)return;let C=i==="add-l1"?e.l1:e.l2.filter(U=>U.Level1&&U.Level1.Id===e.selectedL1);if(C.some(U=>U.Title===c)){I("warn","\u300C"+c+"\u300D\u306F\u65E2\u306B\u767B\u9332\u3055\u308C\u3066\u3044\u307E\u3059");return}p("\u8FFD\u52A0",async()=>{let U={Title:c,SortOrder:m(C),Active:!0};i==="add-l2"&&(U.Level1Id=e.selectedL1),await _(i==="add-l1"?y:h,U),await E(),I("ok","\u300C"+c+"\u300D\u3092\u8FFD\u52A0\u3057\u307E\u3057\u305F")});return}if(T)if(i==="rename"){let g=await M({title:"\u540D\u79F0\u5909\u66F4",inputValue:T.Title,okLabel:"\u4FDD\u5B58"});if(!g||g===T.Title)return;p("\u540D\u79F0\u5909\u66F4",async()=>{await d(z,L,{Title:g}),await E()})}else if(i==="del"){if(f==="l1"){let c=e.l2.filter(C=>C.Level1&&C.Level1.Id===L);if(c.length){I("warn","\u300C"+T.Title+"\u300D\u306B\u306F\u7B2C2\u968E\u5C64\u304C "+c.length+" \u4EF6\u3042\u308A\u307E\u3059\u3002\u5148\u306B\u7B2C2\u968E\u5C64\u3092\u524A\u9664\u3057\u3066\u304F\u3060\u3055\u3044");return}}if(!await M({title:"\u524A\u9664\u306E\u78BA\u8A8D",message:"\u300C"+T.Title+"\u300D\u3092\u524A\u9664\u3057\u307E\u3059\u3002\u3053\u306E\u64CD\u4F5C\u306F\u5143\u306B\u623B\u305B\u307E\u305B\u3093\u3002",okLabel:"\u524A\u9664\u3059\u308B",danger:!0}))return;p("\u524A\u9664",async()=>{await k(z,L),await E(),I("ok","\u300C"+T.Title+"\u300D\u3092\u524A\u9664\u3057\u307E\u3057\u305F")})}else(i==="up"||i==="down")&&p("\u4E26\u3079\u66FF\u3048",async()=>{await S(z,F,T,i==="up"?-1:1),await E()})}),B(),p("\u8AAD\u8FBC",E),window.__permreg={state:e,build:R},pt(R)})();})();
