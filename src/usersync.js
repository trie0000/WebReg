...
async function ensureUserList(log) {
  ...
  // 基本列は毎回 ensure(冪等)。リスト乗っ取りや「列も初期化」で消えていても作り直す
...
async function syncEnglishUserList(state, log) {
  ...
  // 基本列は毎回 ensure(冪等)。「列も初期化」等で消えていても作り直す
...
// 読み取り専用の集計列を新規/編集フォームから隠す。
// 選択肢列(変更区分/権限)の書式には一切触れない: 一部テナントでは選択肢列に CustomFormatter を
// 当てると一覧・フォーム双方の描画が壊れるため。色付けは SP 標準の「選択肢ごとの色」機能で行う
// (列の編集で各選択肢に色を設定。フォームを壊さず一覧でもピル表示される)。
// listTitle 省略時は日本語の利用者一覧。lang は呼び出し側互換のため受けるが未使用。
